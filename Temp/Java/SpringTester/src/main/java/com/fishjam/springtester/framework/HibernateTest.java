 ...
 * 全自动 -- 程序员只需定义好POJO 到数据库表的映射关系，会根据制定的存储逻辑，自动生成对应的 SQL 并调用 JDBC 接口加以执行
 * 缺陷：
 *   1.限制您所使用的对象模型。(例如，一个持久性类不能映射到多个表)
 *   2.对于无法获取表结构的数据无能为力(比如 数据库出于安全考虑的原因，不能完全访问)
 *   3.有些开发规范(比如银行)中要求，所有牵涉到业务逻辑部分的数据库操作，必须在数据库层由存储过程实现
 *   4.对性能要求严格的场景不适用 -- 必须使用经过高度优化的SQL语句（或存储过程）才能达到系统性能设计指标
 *
 * Hibernate运行在两种环境下：
