...
PBYTE __cdecl _InlineHookGetRealCode(PBYTE pbCode, LONG MinCheckSize)
{
...
#else
                //偏移是32位有符号方式，可以正负方向偏移(如 Fasoo 中 0xFF25 0xFFFFFFF2 )
                //PBYTE pbTargetold = pPtrOffset + 6 + (ULONG)(*(PBYTE *)&pPtrOffset[2]);
                PBYTE pbTarget = pPtrOffset + 6 + *(INT32 *)&pPtrOffset[2];
#endif 
    ...
    {
        HOOK_TRACE(("ERROR _InlineHookGetRealCode for pbCode=%p, pPtrOffset=%p\n", 
			pbCode, pPtrOffset));
        return NULL;
    }
}
