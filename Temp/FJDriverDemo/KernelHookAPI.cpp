...
public:
    ...
    VOID InitValue()
    {
        ...
        KeInitializeSpinLock(&m_SplinLock);
    ...
    BOOL AddCreatedDisplayDC(HDC hDC)
    {
        ...
        if (hDC)
        {
            ...
            KeReleaseSpinLock(&m_SplinLock, irql);
        }
        if (!bRet)
        {
    ...
    NTSTATUS _InitCallNumberX86(ULONG versionCode, ULONG buildNumber)
    {
    ...
    NTSTATUS _InitCallNumberX64(ULONG versionCode, ULONG buildNumber)
    {
        NTSTATUS status = STATUS_SUCCESS;
        switch (versionCode)
        {
    ...
    NTSTATUS InitCallNumber()
    {
        ...
        KdPrint(("RtlGetVersion, version is %d, buildNumber=%d, platformId=%d, csd=%ws\n", 
            versionCode, osVersionInfo.dwBuildNumber, osVersionInfo.dwPlatformId, osVersionInfo.szCSDVersion));
...
#if defined(_M_IX86)
        FNT_VERIFY(_InitCallNumberX86(versionCode, osVersionInfo.dwBuildNumber));
#elif defined(_M_AMD64)
        FNT_VERIFY(_InitCallNumberX64(versionCode, osVersionInfo.dwBuildNumber));
#else
        error unsupport OS
#endif 
        return status;
    ...
    NTSTATUS RefreshMemoryDC(HDC hdcDst)
    {
        ...
        if (m_hMemoryDC)
        {
            pOrigNtGdiDeleteObjectApp(m_hMemoryDC);
            m_hMemoryDC = NULL;
        }
        if (m_hBitmap)
        {
            pOrigNtGdiDeleteObjectApp(m_hBitmap);
            m_hBitmap = NULL;
        }
        m_hMemoryDC = pOrigNtGdiCreateCompatibleDC(hdcDst);
        m_hBitmap = pOrigNtGdiCreateCompatibleBitmap(hdcDst, cx, cy);
...
BOOL DrawFilterWnd(HDC hDCDest)
{
    ...
    NT_ASSERT(hRgn != NULL);
    HBRUSH hBrush = g_pDriverHookApiInfos->pOrigNtGdiCreateSolidBrush(RGB(111, 0, 0), NULL);
    NT_ASSERT(hBrush != NULL);

    bRet = g_pDriverHookApiInfos->pOrigNtGdiFillRgn(hDCDest,hRgn, hBrush);  
    KdPrint(("NtGdiFillRgn hRgn in DrawFilterWnd [%d,%d]-[%d,%d] return %d\n", 
        rcWnd.left, rcWnd.top, rcWnd.right, rcWnd.bottom, bRet));
                     ...
                     ULONG  ROP, ULONG crBackColor, ULONG fl)
{
    ...
    __try
    {
        ...
        if (pOrigNtGdiBitBlt)
        {
            ...
            {
                bRet = (pOrigNtGdiBitBlt)(hdcDst, XDest, YDest, Width, Height, hdcSrc, XSrc, YSrc, ROP, crBackColor, fl);
            }
        }
    }
    __finally
