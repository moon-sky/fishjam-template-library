...
*
* 当创建一个新进程时，很多早期的创建工作（创建进程对象、进程空间、建立初始线程、通知子系统等）都是在父进程的环境下完成的。
*   初始线程真正在新进程环境下执行是从内核态的KiThreadStartup 开始的。KiThreadStartup 将线程的IRQL（中断级别）降到APC 级别后
*   调用PspUserThreadStartup 来为线程在用户态执行做准备。
*
* 内核态执行用户态的代码 -- 初始化一个异步过程调用(APC)并插入到APC队列中
******************************************************************************************************************/
