...
LRESULT CMakerCompletePage::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
...
void CMakerCompletePage::InitializeFont(void)
{
    // Set the font
    CLogFont logFont;
    CClientDC dc(m_hWnd);
...
void CMakerCompletePage::InitializeControls(void)
{
...
void CMakerCompletePage::InitializeValues(void)
{
}

void CMakerCompletePage::UpdateSummary(void)
{
    CString path = m_pMakerWizardInfo->GetPath();
    bool recurse = m_pMakerWizardInfo->GetRecurse();
    CString filter = m_pMakerWizardInfo->GetFilter();
    MakerWizardOutputType outputType = m_pMakerWizardInfo->GetOutputType();
    ...
    case eOutput_SaveToFile:
        {
            CString outputFileName = m_pMakerWizardInfo->GetOutputFileName();
            MakerWizardOutputFileEncoding outputFileEncoding = m_pMakerWizardInfo->GetOutputFileEncoding();
...
int CMakerCompletePage::OnSetActive()
{
    this->SetWizardButtons(PSWIZB_BACK | PSWIZB_FINISH); //PSWIZB_FINISH);
...
int CMakerCompletePage::OnWizardBack()
{
    // 0  = goto previous page
    // -1 = prevent page change
    // >0 = jump to page by dlg ID
    return m_pMakerWizardInfo->FindPreviousPage(IDD);
}

INT_PTR CMakerCompletePage::OnWizardFinish()
{
    // We could either do the work here, or in the place that
    // called DoModal on our Sheet (which for this example is CMakerWizard).
    ...
    bool success = m_pMakerWizardInfo->FinishWizard(m_hWnd);
...
void CMakerCompletePage::OnHelp()
{
