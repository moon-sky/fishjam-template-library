...
/////////////////////////////////////////////////////////////////////////////
BOOL CQuantizer::ProcessImage (unsigned int Width, unsigned int Height, 
                               int ColorRes, BYTE* pBmpData)
{
    BYTE r,	g, b, a;
    int	i, j;

    //BITMAPINFOHEADER ds;
    //memcpy(&ds, &bmpHeader, sizeof(ds));
    int effwdt = ((((ColorRes * Width ) + 31) / 32) * 4);

    int	nPad = effwdt - (((Width *	ColorRes) + 7) / 8);

    BYTE* pbBits = pBmpData; //(BYTE*)hImage + *(DWORD*)hImage + ds.biClrUsed * sizeof(RGBQUAD);
