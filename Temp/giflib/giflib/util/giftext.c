...
static char
    *CtrlStr =
    ...
    //-c ColorMapFlag -- 显示颜色映射表的二进制值

static void PrintCodeBlock(GifFileType *GifFile, GifByteType *CodeBlock, bool Reset, bool bPrintCode);
...
int main(int argc, char **argv)
{
    ...
    char *GifFileName = NULL, **FileName = NULL;
    GifPixelType *Line = NULL;
    GifRecordType RecordType = UNDEFINED_RECORD_TYPE;
    GifByteType *CodeBlock = NULL, *Extension = NULL;
    GifFileType *GifFile = NULL;

    if ((Error = GAGetArgs(argc, argv, CtrlStr, &GifNoisyPrint, &ColorMapFlag, &EncodedFlag,
		&LZCodesFlag, &PixelFlag, &RawFlag, &HelpFlag, &NumFiles, &FileName)) != false 
        || (NumFiles > 1 && !HelpFlag)) {
        if (Error){
	        GAPrintErrMsg(Error);
        }
    ...
    if (RawFlag) {
	    ...
	    printf("\tColorResolution = %d, BitsPerPixel = %d, BackGround = %d, Aspect = %d.\n",
	           ...
	           GifFile->AspectByte,
               GifFile->ImageCount, GifFile->ExtensionBlockCount
               );
        //printf("\tImageCount = %d, ExtensionBlockCount = %d.\n", 
        //    GifFile->ImageCount, GifFile->ExtensionBlockCount);

        if (GifFile->SColorMap){
	        printf("\tHas Global Color Map. ColorCount=%d, BitsPerPixel=%d, Sort Flag: %s\n\n", 
                GifFile->SColorMap->ColorCount, GifFile->SColorMap->BitsPerPixel,
	        ...
	        case IMAGE_DESC_RECORD_TYPE:
                {
                    ...
                    if (!RawFlag) { //-r
                        printf("\nImage #%d:\n\n\tImage Size : Left = %d, Top = %d, Width = %d, Height = %d.\n",
                            ImageNum++, GifFile->Image.Left, GifFile->Image.Top, GifFile->Image.Width, GifFile->Image.Height);
                        printf("\tImage is %s", GifFile->Image.Interlace ? "Interlaced" : "Non Interlaced");
                        ...
                        if (GifFile->Image.ColorMap){
                            printf("\tImage Has Color Map. ColorCount=%d, BitsPerPixel=%d, Sort Flag: %s\n", 
                                GifFile->Image.ColorMap->ColorCount, GifFile->Image.ColorMap->BitsPerPixel,
                    ...
                    printf("\tImage LZ compressed Codes (Code Size = %d):\n", CodeSize);

                    if (EncodedFlag) // -e
                    { 
                        PrintCodeBlock(GifFile, CodeBlock, true, EncodedFlag);
                        while (CodeBlock != NULL) {
                            if (DGifGetCodeNext(GifFile, &CodeBlock) == GIF_ERROR) {
                                PrintGifError(GifFile->Error);
                                exit(EXIT_FAILURE);
                            }
                            PrintCodeBlock(GifFile, CodeBlock, false, EncodedFlag);
                        }
                    }
                    else if (LZCodesFlag) { //-z
                        PrintLZCodes(GifFile);
                    }
                    else if (PixelFlag) {   //-p
                        Line = (GifPixelType *) malloc(GifFile->Image.Width * sizeof(GifPixelType));
...
static void PrintCodeBlock(GifFileType *GifFile, GifByteType *CodeBlock, bool Reset, bool bPrintCode)
{
    ...
    if (Reset || CodeBlock == NULL) {
	    if (CodeBlock == NULL) {
            ...
            if (GifFile->Image.ColorMap){
		        NumBytes = ((((long) GifFile->Image.Width) * GifFile->Image.Height) * GifFile->Image.ColorMap->BitsPerPixel) / 8;
            }
            else{
		        NumBytes = ((((long) GifFile->Image.Width) * GifFile->Image.Height) * GifFile->SColorMap->BitsPerPixel) / 8;
            }
...
static void PrintExtBlock(GifByteType *Extension, bool Reset)
{
    ...
    if (Extension != NULL) {
	    ...
	    Len = Extension[0];
	    for (i = 1; i <= Len; i++) {
            (void)_snprintf(&HexForm[CrntPlace * 3], 3, " %02x", Extension[i]);
	        (void)_snprintf(&AsciiForm[CrntPlace], 3, "%c", MAKE_PRINTABLE(Extension[i]));
	        if (++CrntPlace == 16) {
		        HexForm[CrntPlace * 3] = 0;
		        AsciiForm[CrntPlace] = 0;
...
static void PrintPixelBlock(GifByteType *PixelBlock, int Len, bool Reset)
{
    ...
    for (i = 0; i < Len; i++) {
	    (void)_snprintf(&HexForm[CrntPlace * 3], 3, " %02x", PixelBlock[i]);
	    (void)_snprintf(&AsciiForm[CrntPlace], 3, "%c", MAKE_PRINTABLE(PixelBlock[i]));
	    if (++CrntPlace == 16) {
	        HexForm[CrntPlace * 3] = 0;
	        AsciiForm[CrntPlace] = 0;
	        printf("\n%05lx: %-49s  %-17s", ExtCount, HexForm, AsciiForm);
