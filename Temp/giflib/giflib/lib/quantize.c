                         ...
                         GifColorType * ColorMap, GifByteType * GIFBuffer)
{
    ...
    case 24://RGB
        {
            for (i = 0; i < nLength; i++) {
                RedInput[i] = RGBBuffer[i * 3 + 2];         // & 0xff;
                GreenInput[i] = (RGBBuffer[i * 3 + 1]); // >> 8) & 0xff;
                BlueInput[i] = (RGBBuffer[i * 3]);   // >> 16) & 0xff;
            }
            break;
        }
    case 32: //RGBA
        {
            for (i = 0; i < nLength; i++) {
                RedInput[i] = RGBBuffer[i * 4 + 2] & 0xff;
