...
class CButtonSSL : public CButton {
public:
// Construction
	CButtonSSL();
// Destruction
...
// Check Box Functions
	...
	int	 GetSSLCheck ();
// Menu Functions
	BOOL SetSSLButtonMenu (UINT nResourceID, UINT nSubMenuPos = 0);
	BOOL SetSSLButtonMenu (LPCTSTR lpszResourceName, UINT nSubMenuPos = 0);
...
// Miscellaneous Functions
	...
	void SetSSLButtonURL (UINT nResourceID);
...
protected:
	...
	void SetSize ();
	void CmdRouteMenu(CMenu* pPopupMenu); // Added by Abin, 11/07/2003

	CMenu			m_menu;
	UINT			m_nSubMenuPos; // Added by Abin, 11/07/2003, submenu index
...
protected:
	// Enables default state handling
	void EnableDefault(BOOL bEnable);
	// Determine if we should have the default state
	BOOL IsDefault();
	// Determines whether we are a cehckbox, radio button or push button
	UINT GetControlType();
