						   ...
						   LPARAM lParam ) // second message parameter
{
	...
	case WM_CTLCOLOREDIT:
		...
		{
			HDC hdc = (HDC) wParam;
			if (GetDlgItem (hDlg,IDC_EDIT_TEXT) == (HWND) lParam)
			{
...
//	HWND hWnd = ::WindowFromPoint (pt);
	...
	if (hParent != NULL) 
	{
		...
		::SetDlgItemText (hDlg, IDC_EDIT_PAR_HANDLE, ch);		// parent handle

		::GetWindowText (hParent, ch, 128);
		::SetDlgItemText (hDlg, IDC_EDIT_PAR_TEXT, ch);    // parent text

		::GetClassName (hParent, ch, 128);
		::SetDlgItemText (hDlg, IDC_EDIT_PAR_CLASS, ch);    // parent class name
	}
	else
	{
		::SetDlgItemText (hDlg, IDC_EDIT_PAR_HANDLE,  "N/A");
		::SetDlgItemText (hDlg, IDC_EDIT_PAR_TEXT, "N/A");
		::SetDlgItemText (hDlg, IDC_EDIT_PAR_CLASS, "N/A");
	}
	//	
	...
	::SetDlgItemText (hDlg, IDC_EDIT_HANDLE, ch);      // window handle

	int nID = GetWindowLong(hWnd, GWL_ID);
	sprintf (ch, "%d", nID);
	::SetDlgItemText (hDlg, IDC_EDIT_CONTROL_ID, ch);      // control ID
	
	DWORD style = (DWORD)::GetWindowLong (hWnd, GWL_STYLE);
	sprintf (ch, "0x%08X", style);	
	::SetDlgItemText (hDlg, IDC_EDIT_STYLE, ch);      // window style

	RECT rc; 
	::GetWindowRect(hWnd, &rc);
	sprintf (ch, "(%d, %d)-(%d, %d) %dx%d", rc.left, rc.top, rc.right, rc.bottom,
													  rc.right - rc.left, rc.bottom - rc.top);
	::SetDlgItemText (hDlg, IDC_EDIT_RECT, ch);      // RECT

	sprintf (ch, "0x%08X", TID);
	::SetDlgItemText (hDlg, IDC_EDIT_THREADID, ch);      // thread ID

	sprintf (ch, "0x%08X", PID);
	::SetDlgItemText (hDlg, IDC_EDIT_PROCESSID, ch);      // process ID


	::GetClassName (hWnd, ch, 128);
	::SetDlgItemText (hDlg, IDC_EDIT_CLASS, ch);      // class name
	...
	::SetDlgItemText( hDlg, IDC_EDIT_TEXT,ch );         // window text (password)
}
