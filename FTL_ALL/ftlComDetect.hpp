...
namespace FTL
{
    ...
    public:
        static HRESULT DumpInterfaceInfo(IUnknown* pUnknown)
        {
			...
			//获取声明的资源或接口(类型的数量)
			INT nImplTypeFlags = 0;
			for (int i = 0; i < pta->cImplTypes; i++)
			{
				nImplTypeFlags = 0;
				COM_VERIFY(pTypeInfo->GetImplTypeFlags(i, &nImplTypeFlags)); //取得序号指定的类型标志位
				if (SUCCEEDED(hr))
				{
					FTLTRACEEX(FTL::tlTrace, TEXT("\t\t\tImplTypes[%d in %d] = 0x%x\n"), i, pta->cImplTypes, nImplTypeFlags);
					
					//判断是否是资源
					if ((nImplTypeFlags & IMPLTYPE_MASK) == IMPLTYPE_DEFAULTSOURCE)
					{
						HREFTYPE hRefType = 0;
						COM_VERIFY(pTypeInfo->GetRefTypeOfImplType( i, &hRefType)); //先取得声明类型的句柄
						if (SUCCEEDED(hr))
						{
							//Events:
					...
					//判断是否是接口
					else if ((nImplTypeFlags & IMPLTYPE_MASK) == IMPLTYPE_DEFAULTINTERFACE)
					{
						HREFTYPE hRefType = 0;
						COM_VERIFY(pTypeInfo->GetRefTypeOfImplType( i, &hRefType)); //先取得声明类型的句柄
						if (SUCCEEDED(hr))
			...
			for (int i = 0; i < pta->cVars; ++i)
			{
				...
				COM_VERIFY(pTypeInfo->GetVarDesc(i, &pVarDesc));
				BSTR   bstrName = NULL;
				BSTR   bstrNameOfDocument = NULL;
				UINT nNames = 0;
				COM_VERIFY(pTypeInfo->GetNames( pVarDesc->memid, &bstrName, 1, &nNames));
				COM_VERIFY(pTypeInfo->GetDocumentation(pVarDesc->memid, &bstrNameOfDocument,NULL,NULL,NULL));
				FTLTRACEEX(FTL::tlTrace,TEXT("\t\t\tVar[%d in %d] = %s(%s)\n"), i, pta->cVars, 
					OLE2T(bstrName), OLE2T(bstrNameOfDocument));
				SysFreeString(bstrNameOfDocument);
	...
	class CFProvideClassInfoDump
	{
	public:
		static HRESULT DumpInterfaceInfo(IUnknown* pUnknown)
		{
			HRESULT hr = E_FAIL;
			ATL::CComQIPtr<IProvideClassInfo> spProvideClassInfo(pUnknown);
			if (spProvideClassInfo)
			{
				CComPtr<ITypeInfo>	spTypeInfo;
				COM_VERIFY(spProvideClassInfo->GetClassInfo(&spTypeInfo));
				if (spTypeInfo)
				{
					COM_VERIFY(CFTypeInfoDump::DumpInterfaceInfo(spTypeInfo));
				}
...
#  if(_WIN32_IE >= _WIN32_IE_IE70)
	...
	public:
		...
		{
			HRESULT hr = S_OK;
			ATL::CComQIPtr<IUri> spUri(pUnknown);
			if (spUri)
			{
				for (DWORD dwProperty = Uri_PROPERTY_STRING_START; dwProperty <= Uri_PROPERTY_STRING_LAST; ++dwProperty)
				{
					BSTR bstrProperty = NULL;
					COM_VERIFY(spUri->GetPropertyBSTR((Uri_PROPERTY)dwProperty, &bstrProperty, 0));
					if (S_OK == hr)
					{
						FTLTRACEEX(FTL::tlTrace, TEXT("%d = %s\n"), dwProperty, COLE2CT(bstrProperty));
						SysFreeString(bstrProperty);
					}
					else
					{
						ATLASSERT(NULL == bstrProperty);
					}
				}
#if 0
				...
				{	
					DWORD dwValue = 0;
					COM_VERIFY(spUri->GetPropertyDWORD((Uri_PROPERTY)dwProperty, &dwValue, 0));
				}
#endif 
			}
			return S_OK;
...
#  endif //if(_WIN32_IE >= _WIN32_IE_IE70)
		...
		DWORD dwInterfaceCount, REFIID checkedRIIF, LPCTSTR pszInterfaceName)
	{
		...
		UNREFERENCED_PARAMETER(pUnknwon);
		UNREFERENCED_PARAMETER(dwInterfaceCount);
		UNREFERENCED_PARAMETER(checkedRIIF);
		UNREFERENCED_PARAMETER(pszInterfaceName);
...
# ifdef __INetFwRule2_INTERFACE_DEFINED__
...
# ifdef __INetFwProduct_INTERFACE_DEFINED__
			DETECT_INTERFACE_ENTRY(INetFwProduct)
# endif //__INetFwProduct_INTERFACE_DEFINED__
# ifdef __INetFwProducts_INTERFACE_DEFINED__
			DETECT_INTERFACE_ENTRY(INetFwProducts)
# endif //__INetFwProducts_INTERFACE_DEFINED__
            ...
            DETECT_INTERFACE_ENTRY_EX(IProvideClassInfo, CFProvideClassInfoDump) //可以通过GetClassInfo成员函数提供对象类型信息
...
#if(_WIN32_IE >= _WIN32_IE_IE70)
			DETECT_INTERFACE_ENTRY_EX(IUri, CFUriDump)  //可以通过 CreateUri 函数创建改接口，然后获得Uri的各种属性
			DETECT_INTERFACE_ENTRY(IUriContainer)
			DETECT_INTERFACE_ENTRY(IUriBuilder)
			DETECT_INTERFACE_ENTRY(IUriBuilderFactory)
