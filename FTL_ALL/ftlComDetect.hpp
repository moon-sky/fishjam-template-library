    ...
    DWORD CFComDetect::CoDetectInterfaceFromList(IUnknown* pUnknown, REFIID checkRIID, ComDetectType detectType)
    {
        ...
        BEGIN_DETECT_INTERFACE()
...
#if INCLUDE_DETECT_DTE
            {
                ...
                DETECT_INTERFACE_ENTRY(Commands)   //管理所有的动作，一个动作可由多种方式触发
                DETECT_INTERFACE_ENTRY(Command)    //动作 -- 注意和触发动作的用户界面(CommandBarControl)的区别
            ...
            }
            {
                ...
                DETECT_INTERFACE_ENTRY(ToolWindows) //管理工具窗口，如 Solution Explorer、Error List等
...
#pragma TODO(will ambiguous with oleacc.h 556 --IAccessible)
                ...
                DETECT_INTERFACE_ENTRY(CommandBarControl)       //菜单项(可再实例化为 XXXButton/Combox 等)，如 Tools 菜单条中的 Options
                ...
                DETECT_INTERFACE_ENTRY(CommandBar)          //一个特定的菜单、工具条
                DETECT_INTERFACE_ENTRY(CommandBarControls)  //管理菜单、工具条上的按钮，如 Tools 中的多个菜单项
                DETECT_INTERFACE_ENTRY(_CommandBars)    //所有的菜单、工具条等，共约300多个
