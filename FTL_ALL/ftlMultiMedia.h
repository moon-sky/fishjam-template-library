...
*     开源、完全免费的图像操作类库，可快捷地存取、显示、转换各种图像。
...
*       TransparentColorIndex(透明色索引, 1Byte) -- 当图像解码时，如果遇到透明索引，则显示设备的相关象素不被改变(跳过不解码)，继续处理下一个象素
...
* 
* 优化:
*   1.帧内容相同时，增加延迟，而非增加新的帧
*   2.比较相邻帧查分保存，从而减少文件体积(编辑界面中绿色区域就是会被自动删除的相同画面)
*   3.编辑帧: 可添加、删除、更改延迟，
*   4.编辑帧内容，如添加文字 等
* 
* Gif录制软件及其功能
