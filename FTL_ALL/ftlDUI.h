...
*     GetInterface -- 根据传入的字符串，传出指向控件的指针，如果没有对应的控件类型，则返回NULL
*     SetAttribute -- 从XML中解析出控件的各个属性，传入字符串的属性名称和值进行设置
*   CContainerUI -- 容器基类，其内部用一个数组(m_items)来保存所有的CControlUI的对象，所有的容器都支持滚动条
*                   TODO：绘制控制(不在显示区域的不绘制)，控件是否知道自己不在显示区好进行优化
*     FindControl -- 查找子控件
*     Remove/RemoveAt -- 移除子控件
*     SetAutoDestroy -- 控制是否自动销毁
*   基本控件(Control目录下) -- 
...
*     CTabLayoutUI -- Tab 排列
* 
*   CRenderEngine -- 很多可以通用的静态绘制代码都在这里。比如 九宫格拉伸图片，平铺图片等
*   CPaintManagerUI -- 控件绘制管理，对应于一个Windows的原生窗口。
*     Init -- 通过原生HWND进行初始化
*     FindControl -- 
...
*
*   CWindowWnd -- 原生窗口控制,通过该类将原生窗口的消息分发给其派生类，最后传给整个控件体系。
*     HandleMessage -- 基本的消息分发
*     OnFinalMessage -- 通常new出来的窗体实例需要在这个方法中 delete this
*   WindowImplBase -- DUI为了简化使用而提供的一个封装类，提供了常用的一些功能，★通常用户的各个窗体就是从该类继承★，
*     CPaintManagerUI m_PaintManager;  -- 保护成员变量，对该窗体的绘制进行管理，
*     HandleMessage -- 进行一些基本的消息处理，子类可以重载对应的消息处理虚函数(采用了虚函数的方式实现消息映射)
*     GetWindowClassName -- 子类必须重载实现的纯虚函数
* 
* 消息(事件)处理方式 -- 
*   消息映射(通过字符串来区分的，需要重 CNotifyPump 及其子类继承?), 最简单的响应函数的类型是 void CMyXXX::OnClick( TNotifyUI &msg );
*     头文件中：  DUI_DECLARE_MESSAGE_MAP
*     实现文件中：DUI_BEGIN_MESSAGE_MAP -> DUI_ON_MSGTYPE(DUI_MSGTYPE_CLICK,OnClick) -> DUI_END_MESSAGE_MAP
*   IDialogBuilderCallback -- 
*   INotifyUI -- 最基本的消息处理接口，使用 TNotifyUI 结构传递消息
*     TNotifyUI:
*       sType -- DUI_MSGTYPE_XXX, 如 DUI_MSGTYPE_CLICK
*       sVirtualWnd -- 通过 AddVirtualWnd 加入时的虚拟窗体名字
*       pSender -- 发送通知的控件，一般通过其 GetName 来判断
*   IMessageFilterUI -- PreMessageFilter，消息预处理
*   ITranslateAccelerator -- 
*   CEventSource -- 事件代理 
*     OnNotify += MakeDelegate(this, &CFrameWindowWnd::OnHChanged);
*   CNotifyPump -- 虚窗体控制(比如WindowImplBase 中内嵌的 Page页)，可通过 AddVirtualWnd 加入，然后通过 TNotifyUI::sVirtualWnd 递归找到对应的虚窗体进行消息处理
*
...
* 
* XML资源(没有 DTD/Schema 的验证文档?)
*   Window -- 最外层的窗体控件，其中有 Font、Default(VScrollBar/HScrollBar) 等子节点定义属性
*   VerticalLayout/HorizontalLayout/TabLayout -- 布局控件，其子控件按照指定的方式自动布局
