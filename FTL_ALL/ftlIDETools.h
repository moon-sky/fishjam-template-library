...
#pragma warning ( default: 4244 )
	...
	*   分布式(Git) -- 开发者可以本地离线工作(提交、合并、创建分支等，每个开发者机器上都有一个服务器的数据库)，然后本地修改后生成补丁发送给主开发者，
	*     主开发者确认后合并到公共服务器
	* 
    *   EGit -- Eclipse 下的 Git Plugin
    *   TortoiseGit -- Windows 下的 Git Plugin
    *   GitHub -- 可与 Hudson 连接?
    *   想开发一个 nGit(参考 Git 和 Gemit的使用，如在VS里打开IE View)
	*   相关命令(git, 使用 git help 命令 可以显示详细的HTML帮助)
	*     add 文件 -- 把想提交的文件add上
	*     branch -- 分支操作，如 创建分支，查看分支，删除分支
	*     checkout -- 切换分支
	*     clone 远程目录 本地目录 -- 从服务器上克隆数据库（包括代码和版本信息）到单机上
	*     commit -- 提交到本地数据库
	*     config -- 配置相关信息，例如email和name
	*     fetch -- 从服务器下载数据库，并放到新分支，不跟自己的数据库合并
	*     init -- 创建一个数据库
	*     log -- 查看版本历史
	*     merge -- 合并分支，把目标分支合并到当前分支
	*     pull -- 从服务器下载数据库，并跟自己的数据库合并
	*     reset -- 恢复到之前的版本
	*     show -- 查看版本号对应版本的历史。如果参数是HEAD查看最新版本
	*     whatchanged -- 查看两个分支的变化
