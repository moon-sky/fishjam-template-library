...
namespace FTL
{
	LPCTSTR CFControlUtil::GetEditNotifyCodeString(CFStringFormater& formater, DWORD iNotify, void *pv)
	{
		switch(iNotify)
		{
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_SETFOCUS);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_KILLFOCUS);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_CHANGE);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_UPDATE);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_ERRSPACE);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_MAXTEXT);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_HSCROLL);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_VSCROLL);
			{
		case EN_MSGFILTER:
				MSGFILTER* pMsgFilter = (MSGFILTER*)pv;
				formater.Format(TEXT("EN_MSGFILTER, pMsgFilter, msg=%d, wParam=0x%x, lParam=0x%x"),
					pMsgFilter->msg, pMsgFilter->wParam, pMsgFilter->lParam);
				break;
			}
			{
		case EN_REQUESTRESIZE:		//EN_ALIGN_RTL_EC
				REQRESIZE* pReqResize = (REQRESIZE*)pv;
				formater.Format(TEXT("EN_REQUESTRESIZE, ReqResize.rc=(%d,%d)-(%d,%d), %dx%d"),
					pReqResize->rc.left, pReqResize->rc.top, pReqResize->rc.right, pReqResize->rc.bottom,
					pReqResize->rc.right - pReqResize->rc.left, pReqResize->rc.bottom - pReqResize->rc.top);
			break;
			}
			{
		case EN_SELCHANGE:
				SELCHANGE* pSelChange = (SELCHANGE*)pv;
				formater.Format(TEXT("EN_SELCHANGE, SelType=%d, From %d to %d"),
					pSelChange->seltyp, pSelChange->chrg.cpMin, pSelChange->chrg.cpMax);
				break;
			}
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_DROPFILES);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_PROTECTED);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_CORRECTTEXT);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_STOPNOUNDO);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_IMECHANGE);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_SAVECLIPBOARD);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_OLEOPFAILED);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_OBJECTPOSITIONS);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_LINK);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_DRAGDROPDONE);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_PARAGRAPHEXPANDED);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_PAGECHANGE);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_LOWFIRTF);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_ALIGNLTR);
			HANDLE_CASE_TO_STRING_FORMATER(formater, EN_ALIGNRTL);
		default:
			formater.Format(TEXT("Unknown Edit Notify Code=%d"), iNotify);
			break;
		}
