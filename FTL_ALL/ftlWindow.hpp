    ...
    LPCTSTR CFMessageInfo::ConvertInfo()
    {
        if (NULL == m_bufInfo[0])
        {
            switch(m_Info)
            {
                ...
                GET_MESSAGE_INFO_ENTRY(WM_DESTROY);  //关闭窗口时,如果是主窗口,必须PostQuitMessage,否则进程还在运行(只是没有窗体)
                GET_MESSAGE_INFO_ENTRY(WM_MOVE);
                GET_MESSAGE_INFO_ENTRY(WM_SIZE);
                GET_MESSAGE_INFO_ENTRY(WM_ACTIVATE); //窗口被激活或失去激活状态, 可用于查看窗体是否最小化，及前一窗体
                GET_MESSAGE_INFO_ENTRY(WM_SETFOCUS);
                GET_MESSAGE_INFO_ENTRY(WM_KILLFOCUS);
                GET_MESSAGE_INFO_ENTRY(WM_ENABLE); //将禁止或激活时
                GET_MESSAGE_INFO_ENTRY(WM_SETREDRAW); //设置窗口是否能重画 
                GET_MESSAGE_INFO_ENTRY(WM_SETTEXT);
                GET_MESSAGE_INFO_ENTRY(WM_GETTEXT);
                GET_MESSAGE_INFO_ENTRY(WM_GETTEXTLENGTH); //得到与一个窗口有关的文本的长度（不包含空字符）
                GET_MESSAGE_INFO_ENTRY(WM_PAINT);
                GET_MESSAGE_INFO_ENTRY(WM_CLOSE);
#ifndef _WIN32_WCE
                GET_MESSAGE_INFO_ENTRY(WM_QUERYENDSESSION); //当用户选择结束对话框或程序自己调用ExitWindows函数
                GET_MESSAGE_INFO_ENTRY(WM_QUERYOPEN);     //当用户窗口恢复以前的大小位置时，把此消息发送给某个图标
                GET_MESSAGE_INFO_ENTRY(WM_ENDSESSION);    //当系统进程发出WM_QUERYENDSESSION消息后，此消息发送给应用程序，通知它对话是否结束
#endif
                GET_MESSAGE_INFO_ENTRY(WM_QUIT);
                GET_MESSAGE_INFO_ENTRY(WM_ERASEBKGND); //当窗口背景必须被擦除时（例在窗口改变大小时）
                GET_MESSAGE_INFO_ENTRY(WM_SYSCOLORCHANGE);    //当系统颜色改变时，发送此消息给所有顶级窗口
...
#  define WM_CTLCOLOR                             0x0019
                ...
                GET_MESSAGE_INFO_ENTRY(WM_ACTIVATEAPP);//属于另一App的窗体将激活时
                GET_MESSAGE_INFO_ENTRY(WM_FONTCHANGE);    //当系统的字体资源库变化时发送此消息给所有顶级窗口
                GET_MESSAGE_INFO_ENTRY(WM_TIMECHANGE);    //当系统的时间变化时发送此消息给所有顶级窗口
                GET_MESSAGE_INFO_ENTRY(WM_CANCELMODE);    //取消系统模式时
                GET_MESSAGE_INFO_ENTRY(WM_SETCURSOR);     //如果鼠标引起光标在某个窗口中移动且鼠标输入没有被捕获时，就发消息给某个窗口
                ...
                GET_MESSAGE_INFO_ENTRY(WM_NEXTDLGCTL);    //此消息发送给某个最小化窗口，仅当它在画图标前它的背景必须被重画
                GET_MESSAGE_INFO_ENTRY(WM_SPOOLERSTATUS); //每当打印管理列队增加或减少一条作业时发出此消息
                GET_MESSAGE_INFO_ENTRY(WM_DRAWITEM);
                GET_MESSAGE_INFO_ENTRY(WM_MEASUREITEM);
                GET_MESSAGE_INFO_ENTRY(WM_DELETEITEM);
                GET_MESSAGE_INFO_ENTRY(WM_VKEYTOITEM);    //由一个LBS_WANTKEYBOARDINPUT风格的发出给它的所有者来响应WM_KEYDOWN消息
                GET_MESSAGE_INFO_ENTRY(WM_CHARTOITEM);    //由一个LBS_WANTKEYBOARDINPUT风格的列表框发送给他的所有者来响应WM_CHAR消息
                GET_MESSAGE_INFO_ENTRY(WM_SETFONT);
                GET_MESSAGE_INFO_ENTRY(WM_GETFONT);
                GET_MESSAGE_INFO_ENTRY(WM_SETHOTKEY);     //应用程序发送此消息让一个窗口与一个热键相关连
                GET_MESSAGE_INFO_ENTRY(WM_GETHOTKEY);     //应用程序发送此消息来判断热键与某个窗口是否有关联
                GET_MESSAGE_INFO_ENTRY(WM_QUERYDRAGICON); //送给最小化窗口，当此窗口将要被拖放而它的类中没有定义图标，应用程序能返回一个图标或光标的句柄，
                //当用户拖放图标时系统显示这个图标或光标
                GET_MESSAGE_INFO_ENTRY(WM_COMPAREITEM);   //发送此消息来判定combobox或listbox新增加的项的相对位置
#if(WINVER >= 0x0500)
                GET_MESSAGE_INFO_ENTRY(WM_GETOBJECT);
#endif
                GET_MESSAGE_INFO_ENTRY(WM_COMPACTING);    //显示内存已经很少了
                ...
                GET_MESSAGE_INFO_ENTRY(WM_POWER);         //当系统将要进入暂停状态时发送此消息
                GET_MESSAGE_INFO_ENTRY(WM_COPYDATA);
                GET_MESSAGE_INFO_ENTRY(WM_CANCELJOURNAL); //当某个用户取消程序日志激活状态，提交此消息给程序
#if(WINVER >= 0x0400)
            case WM_NOTIFY:
                {
                ...
                GET_MESSAGE_INFO_ENTRY(WM_INPUTLANGCHANGEREQUEST);    //当用户选择某种输入语言，或输入语言的热键改变
                GET_MESSAGE_INFO_ENTRY(WM_INPUTLANGCHANGE);   //当平台现场已经被改变后发送此消息给受影响的最顶级窗口
                GET_MESSAGE_INFO_ENTRY(WM_TCARD);         //当程序已经初始化windows帮助例程时发送此消息给应用程序
                GET_MESSAGE_INFO_ENTRY(WM_HELP);          //此消息显示用户按下了F1，如果某个菜单是激活的，就发送此消息个此窗口关联的菜单，
                //  否则就发送给有焦点的窗口，如果当前都没有焦点，就把此消息发送给当前激活的窗口
                GET_MESSAGE_INFO_ENTRY(WM_USERCHANGED);   //当用户已经登入或退出后发送此消息给所有的窗口，当用户登入或退出时系统更新用户的具体
                //  设置信息，在用户更新设置时系统马上发送此消息
                GET_MESSAGE_INFO_ENTRY(WM_NOTIFYFORMAT);  //通过此消息来判断控件是使用ANSI还是UNICODE结构
                GET_MESSAGE_INFO_ENTRY(WM_CONTEXTMENU);   //当用户某个窗口中点击了一下右键就发送此消息给这个窗口
                GET_MESSAGE_INFO_ENTRY(WM_STYLECHANGING); //当调用SETWINDOWLONG函数 将要改变 一个或多个 窗口的风格时发送此消息给那个窗口
                GET_MESSAGE_INFO_ENTRY(WM_STYLECHANGED);  //当调用SETWINDOWLONG函数 改变 一个或多个窗口的风格后发送此消息给那个窗口
                GET_MESSAGE_INFO_ENTRY(WM_DISPLAYCHANGE); //当显示器的分辨率改变后发送此消息给所有的窗口
                GET_MESSAGE_INFO_ENTRY(WM_GETICON);
                GET_MESSAGE_INFO_ENTRY(WM_SETICON);
#endif
                GET_MESSAGE_INFO_ENTRY(WM_NCCREATE);      //当某个窗口第一次被创建时，此消息在WM_CREATE消息发送前发送
                GET_MESSAGE_INFO_ENTRY(WM_NCDESTROY);     //此消息通知某个窗口，非客户区正在销毁
                
                //需要计算窗口客户区的大小和位置时发送，通过处理该消息，可以在窗口大小或位置改变时控制客户区的内容     
                GET_MESSAGE_INFO_ENTRY(WM_NCCALCSIZE);    
                GET_MESSAGE_INFO_ENTRY(WM_NCHITTEST);
                GET_MESSAGE_INFO_ENTRY(WM_NCPAINT);       //程序发送此消息给某个窗口当它（窗口）的框架必须被绘制时
                GET_MESSAGE_INFO_ENTRY(WM_NCACTIVATE);    //此消息发送给某个窗口 仅当它的非客户区需要被改变来显示是激活还是非激活状态
                GET_MESSAGE_INFO_ENTRY(WM_GETDLGCODE);    //发送此消息给某个与对话框程序关联的控件，widdows控制方位键和TAB键使输入进入此控件
                //  通过响应WM_GETDLGCODE消息，应用程序可以把他当成一个特殊的输入控件并能处理它
#ifndef _WIN32_WCE
                GET_MESSAGE_INFO_ENTRY(WM_SYNCPAINT);
#endif
                GET_MESSAGE_INFO_ENTRY(WM_NCMOUSEMOVE);   //非客户区内移动时发送此消息给这个窗口
                ...
                GET_MESSAGE_INFO_ENTRY(WM_DEADCHAR); //当用 translatemessage 函数翻译WM_KEYUP消息时发送此消息给拥有焦点的窗口
                GET_MESSAGE_INFO_ENTRY(WM_SYSKEYDOWN);    //当用户按住ALT键同时按下其它键时提交此消息给拥有焦点的窗口
                GET_MESSAGE_INFO_ENTRY(WM_SYSKEYUP);      //当用户释放一个键同时ALT 键还按着时提交此消息给拥有焦点的窗口
                GET_MESSAGE_INFO_ENTRY(WM_SYSCHAR);       //当WM_SYSKEYDOWN消息被TRANSLATEMESSAGE函数翻译后提交此消息给拥有焦点的窗
                ...
                GET_MESSAGE_INFO_ENTRY(WM_INITDIALOG); //在一个对话框程序被显示前发送此消息给它，通常用此消息初始化控件和执行其它任务
                GET_MESSAGE_INFO_ENTRY(WM_COMMAND);

                //可用于屏蔽屏幕保护和显示器节电模式(SC_SCREENSAVE/SC_MONITORPOWER),返回0
                GET_MESSAGE_INFO_ENTRY(WM_SYSCOMMAND);    //当用户选择窗口菜单的一条命令或当用户选择最大化或最小化时那个窗口会收到此消息

                GET_MESSAGE_INFO_ENTRY(WM_TIMER);     //发生了定时器事件
                GET_MESSAGE_INFO_ENTRY(WM_HSCROLL);   //水平滚动条产生一个滚动事件
                GET_MESSAGE_INFO_ENTRY(WM_VSCROLL);   //垂直滚动条产生一个滚动事件
                GET_MESSAGE_INFO_ENTRY(WM_INITMENU);  //当一个菜单将要被激活时发送此消息，它发生在用户菜单条中的某项或按下某个菜单键，
                //  它允许程序在显示前更改菜单
...
#define WM_SYSTIMER 0x0118
                ...
                GET_MESSAGE_INFO_ENTRY(WM_MENUSELECT); //当用户选择一条菜单项时发送此消息给菜单的所有者（一般是窗口）
                GET_MESSAGE_INFO_ENTRY(WM_MENUCHAR);  //当菜单已被激活用户按下了某个键（不同于加速键），发送此消息给菜单的所有者
                GET_MESSAGE_INFO_ENTRY(WM_ENTERIDLE); //当一个模态对话框或菜单进入空载状态时(处理完一条或几条先前的消息后没有消息它的列队中等待)发送此消息给它的所有者，
#if(WINVER >= 0x0500)
...
#  endif //_WIN32_WINNT >= 0x0500
                ...
                GET_MESSAGE_INFO_ENTRY(WM_CTLCOLORMSGBOX);    //在windows绘制消息框前发送此消息给消息框的所有者窗口，通过响应这条消息，所有者窗口可以
                //  通过使用给定的相关显示设备的句柄来设置消息框的文本和背景颜色
                GET_MESSAGE_INFO_ENTRY(WM_CTLCOLOREDIT);      //当一个编辑型控件将要被绘制时发送此消息给它的父窗口 -- 可以设置编辑框的文本和背景颜色
                GET_MESSAGE_INFO_ENTRY(WM_CTLCOLORLISTBOX);   //当一个列表框控件将要被绘制前发送此消息给它的父窗口 -- 可以设置列表框的文本和背景颜色
                GET_MESSAGE_INFO_ENTRY(WM_CTLCOLORBTN);       //当一个按钮  控件将要被绘制时发送此消息给它的父窗口 -- 可以设置按纽的文本和背景颜色
                GET_MESSAGE_INFO_ENTRY(WM_CTLCOLORDLG);       //当一个对话框控件将要被绘制前发送此消息给它的父窗口 -- 可以设置对话框的文本背景颜色
                GET_MESSAGE_INFO_ENTRY(WM_CTLCOLORSCROLLBAR); //当一个滚动条控件将要被绘制时发送此消息给它的父窗口 -- 可以设置滚动条的背景颜色
                GET_MESSAGE_INFO_ENTRY(WM_CTLCOLORSTATIC);    //当一个静态控件将要被绘制时发送此消息给它的父窗口 -- 可以设置静态控件的文本和背景颜色
                ...
                GET_MESSAGE_INFO_ENTRY(WM_MOUSEWHEEL);        //当鼠标轮子转动时发送此消息个当前有焦点的控件
#endif
                ...
                GET_MESSAGE_INFO_ENTRY(WM_PARENTNOTIFY); //当MDI子窗口被创建或被销毁，或用户按了一下鼠标键而光标在子窗口上时发送此消息给它的父窗口
                GET_MESSAGE_INFO_ENTRY(WM_ENTERMENULOOP); //发送此消息通知应用程序的主窗口 已经进入了菜单循环模式 -- ?
                GET_MESSAGE_INFO_ENTRY(WM_EXITMENULOOP);  //发送此消息通知应用程序的主窗口 已经退出了菜单循环模式 -- ?

#if(WINVER >= 0x0400)
                GET_MESSAGE_INFO_ENTRY(WM_NEXTMENU);
                GET_MESSAGE_INFO_ENTRY(WM_SIZING);        //当用户正在调整窗口大小时发送此消息给窗口；通过此消息应用程序可以监视和修改窗口大小和位置
                GET_MESSAGE_INFO_ENTRY(WM_CAPTURECHANGED);//窗口失去捕获的鼠标时
                GET_MESSAGE_INFO_ENTRY(WM_MOVING);        //当用户在移动窗口时发送此消息
#endif

#if(WINVER >= 0x0400)
                GET_MESSAGE_INFO_ENTRY(WM_POWERBROADCAST);//送给应用程序来通知它有关电源管理事件
                GET_MESSAGE_INFO_ENTRY(WM_DEVICECHANGE);  //当设备的硬件配置改变时发送此消息给应用程序或设备驱动程序
#endif

                GET_MESSAGE_INFO_ENTRY(WM_MDICREATE);     //要求创建一个MDI 子窗口
                ...
                GET_MESSAGE_INFO_ENTRY(WM_MOUSELEAVE); //鼠标移开时，需要用 ON_MESSAGE 响应
#endif
                ...
                GET_MESSAGE_INFO_ENTRY(WM_DRAWCLIPBOARD); //当剪贴板的内容变化时发送此消息给剪贴板观察链的第一个窗口；它允许用剪贴板观察窗口来显示剪贴板的新内容
                GET_MESSAGE_INFO_ENTRY(WM_PAINTCLIPBOARD);//当剪贴板包含CF_OWNERDIPLAY格式的数据并且剪贴板观察窗口的客户区需要重画
                ...
                GET_MESSAGE_INFO_ENTRY(WM_CHANGECBCHAIN); //当一个窗口从剪贴板观察链中移去时发送此消息给剪贴板观察链的第一个窗口
                GET_MESSAGE_INFO_ENTRY(WM_HSCROLLCLIPBOARD);
                GET_MESSAGE_INFO_ENTRY(WM_QUERYNEWPALETTE);   //此消息能使窗口在收到焦点时同时有机会实现他的逻辑调色板
                GET_MESSAGE_INFO_ENTRY(WM_PALETTEISCHANGING);
                GET_MESSAGE_INFO_ENTRY(WM_PALETTECHANGED);
                GET_MESSAGE_INFO_ENTRY(WM_HOTKEY);        //当用户按下由REGISTERHOTKEY函数注册的热键时提交此消息
...
#  ifdef _DDEHEADER_INCLUDED_
                GET_MESSAGE_INFO_ENTRY(WM_DDE_INITIATE);  //一个DDE客户程序提交此消息开始一个与服务器程序的会话来响应那个指定的程序和主题名
