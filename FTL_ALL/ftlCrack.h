...
* 非对称密钥：公钥(PK -- Public Key)加密，私钥(SK -- )解密，加密算法(E)和解密算法(D)也是公开的，机制灵活，但速度慢。
*   基于“由已知加密密钥推导出解密密钥在计算上是不可行的”的密码体制。
*   为减少计算量，在传送信息时，常采用传统加密方法与公钥加密方法相结合的方式？
...
*  
* OpenSSL -- 为网络通信提供安全及数据完整性的一种安全协议，囊括了主要的密码算法、常用的密钥和证书封装管理功能以及SSL协议.
*   BIO -- 不管连接是安全的还是不安全的，OpenSSL 都使用了一个名为 BIO 的抽象库来处理包括文件和套接字在内的各种类型的通信。
**********************************************************************************************************************************/
