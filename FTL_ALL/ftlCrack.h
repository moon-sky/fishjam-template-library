...
*   常见对称加密算法
*     [AES](Advanced Encryption Standard) -- 高级加密标准，是美国联邦政府采用的一种区块加密标准，用于替代原先的DES，目前最流行的私钥数据(对称）加密算法之一。
*     Blowfish
*     CAST
*     DES
*     IDEA
...
*     RC5
*
* 2.非对称密钥(非对称加密算法，常用的是RSA)：公钥(PK -- Public Key)加密，私钥(SK -- )解密，加密算法(E)和解密算法(D)也是公开的，机制灵活，但速度慢。
...
*     EC(椭圆曲线)
*
* 3.信息摘要算法 -- 一种能产生特殊输出格式的算法，无论输入数据如何，输出的密文长度固定，常用于数据完整性校验
*   常见算法
*     DSS/DSS1
*     MD2
*     [MD5] -- 输出16字节
