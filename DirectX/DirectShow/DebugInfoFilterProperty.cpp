...
HRESULT CDebugInfoFilterProperty::OnActivate()
{
	...
	{
		DX_VERIFY_EXCEPT1(m_pFilterInfo->GetConnectedPin(TRUE,&m_pInputConnectedPin),VFW_E_NOT_CONNECTED);
	}
	...
	{
		DX_VERIFY_EXCEPT1(m_pFilterInfo->GetConnectedPin(FALSE,&m_pOutputConnectedPin),VFW_E_NOT_CONNECTED);
	}
	...
	DX_VERIFY(m_pFilterInfo->GetFilterDebugParam(&m_FilterDebugParam));
