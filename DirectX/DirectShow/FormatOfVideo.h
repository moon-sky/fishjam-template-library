...
*       NV12 -- 用于DirectX VA的首选4:2:0像素格式。Intel Media SDK 只支持这种格式（似乎显卡内部就是这种格式）
...
*     H.264 -- 有专利保护问题，其专利保护至少到2028年，可能随时为解码过程支付费用
*     数据流具备严格的语法规则，各标准之间具有兼容性，是一个开放体系，可以对其进行改进，提高其性能。
*   H.264/AVC（Advanced Video Coding), MPEG-4 part10 -- 由ITU-T与ISO/IEC联合进行开发的视频编解码方案，通常音频使用AAC,
...
*   WebM -- Google发布的开源HTML5视频标准(http://www.webmproject.org/), ffmpeg 中对应的编解码器是 libvpx
