...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) 
{
    ...
    PAINTSTRUCT ps; // used in WM_PAINT HDC hdc; // handle to a device context
    // what is the message 
    switch(msg)
    {
    case WM_CREATE: 
...
{ 
    ...
    { 
        ...
        else if (ball_y > (SCREEN_HEIGHT - BALL_SIZE))  // penalize player for missing the ball 
        { 
            // reflect y-axis velocity
            ball_dy=-ball_dy;
            // update position 
        ...
        Draw_Rectangle(paddle_x-8, paddle_y+8, paddle_x+PADDLE_WIDTH-8, paddle_y+PADDLE_HEIGHT+8,0);
        Draw_Rectangle(paddle_x, paddle_y, paddle_x+PADDLE_WIDTH, paddle_y+PADDLE_HEIGHT,PADDLE_COLOR);

        // draw the ball 
        Draw_Rectangle(ball_x-4, ball_y+4, ball_x+BALL_SIZE-4, ball_y+BALL_SIZE+4, 0); 
        ...
        // flip the surfaces 
        DD_Flip(); //游戏图像提交到显示缓冲区，最后通过DD_FLIP()访问在循环结束时在显示屏上显示出来
