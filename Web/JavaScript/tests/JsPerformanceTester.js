...
/*************************************************************************************************************************
* 内存管理
*   delete 可以删除对象实例的属性(如 delete user.name; )，不能删除变量( 如不能 delete user; )
*   如果不需要对象实例的话，可以手动赋值为 null(推荐), 或等待超出函数的生存期，自动释放
*************************************************************************************************************************/
...
//"use strict";
    ...
    var objString = new String("hello");
    ok(typeof (objString) == "object", "new 一个变量，即可使用");

    delete objString;
    equal(typeof (objString), "object", "delete 以释放内存");  //TODO: 和 赋值为 null ?
    objString += " world";
    equal(objString, "hello world", "delete 以后");
    
    objString = null;
    equal(objString, null, "赋值为 null 以后");
});