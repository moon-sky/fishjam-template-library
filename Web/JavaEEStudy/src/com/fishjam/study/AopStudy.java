 ...
 *   注意：继承与委托是最常见的实现重用通用功能的面向对象技术。但是，继承会导致一个脆弱的对象体系(无法更改框架？)，委托可能需要对委托对象
 *            进行复杂的调用。切面提供了取代继承和委托的另一种选择 -- 通过声明的方式定义通用功能以何种方式在何处应用，而无需修改受影响的类。
 *
 * 术语: 
 ...
 *       类加载期：需要特殊的类加载器(ClassLoader)，可以在目标类被引入应用之前增强该目标类的字节码。如 AspectJ 5 的LTW(load-time weaving)。
 *       运行期：   AOP容器为目标对象动态地创建一个代理对象，在应用运行的时候织入，拦截被通知的方法的调用，再将调用转发给真正的目标Bean。如 Spring AOP。
 *       
 * AOP框架
 *   AspectJ -- 支持方法、属性、构造器拦截，但需要特有的AOP语言，在编译时织入
 *   JBoss AOP
 *   Spring AOP -- 只支持方法连接点，使用标准的Java语言，在运行时织入
 *   
 * 使用方式(Spring AOP 为例)：
